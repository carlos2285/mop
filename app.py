...
def vc_percent(df, col, by=None):
    """Return counts and %; if by provided, % is within each by-group."""
    if by is not None:
...
def crosstab_pct(df, r, c, by=None):
    """Crosstab with counts and row % within each 'by' if provided; else global row %."""
    if by is not None:
        out = []
...
def export_xlsx(sheets_dict):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        for sheet, df in sheets_dict.items():
            if isinstance(df, dict):  # multiple tables under same sheet
                startrow = 0
...
if uploaded is None:
    if os.path.exists("data/respuestas.xlsx"):
        df = pd.read_excel("data/respuestas.xlsx")
    elif os.path.exists("data/respuestas.csv"):
...
# p004 subset for applicable blocks
...
with tabC:
    ...
    # Desagregados por componentes del hogar si existen
...
with tabG:
    ...
    for col, label in [(p036,"Percepción de seguridad (p036)"),
                       ...
                       (p035tx,"Problemas identificados (p035tx)")]:  # p035tx suele ser texto/codificado
        if col!="<ninguna>":
            st.markdown(f"**{label}**")
...
with tabI:
    ...
    # Define helpers for % calculations with NA handling
    def pct_true(series):
        s = series.dropna()
        if len(s)==0: return np.nan
        return (s.mean()*100)

    # Map conditions (the user can adjust via mapping later if needed)
    ind_tables = {}
    base = work.copy()

    # % estructuras en mal estado (p005 contiene 'malo' o similar)
    if p005!="<ninguna>":
    ...
    st.caption("Las reglas de indicadores son heurísticas por texto/categoría; podemos ajustar a tu codificación exacta si nos indicas las etiquetas/códigos válidos.")
