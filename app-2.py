...
#   streamlit, pandas, numpy, pydeck, matplotlib, wordcloud, openpyxl, pillow
...
# ---------------------------------------------------------
# Visualización (con tolerancia a paquetes faltantes)
# ---------------------------------------------------------
...
STOPWORDS_ES = {
    ...
    "estuviese","estuvieses","estuviésemos","estuvieseis","estuviesen","siendo",
    "sido","hoy","año","años","mes","meses","día","días","así","cada"
}
...
with tab_mapa:
    ...
    if lat_col == "<ninguna>" or lon_col == "<ninguna>":
        ...
        if len(mdf) == 0:
            ...
            if color_by_sector and "SECTOR" in mdf.columns:
                codes = mdf["SECTOR"].astype("category").cat.codes
...
with tab_nube:
    ...
    if not (MATPLOTLIB_OK and WORDCLOUD_OK):
        st.error("La nube requiere 'matplotlib' y 'wordcloud'. "
                 "Asegúrate de tener el requirements.txt y runtime.txt del proyecto actualizados.")
        if not MATPLOTLIB_OK:
            st.caption(f"matplotlib no disponible: {MATPLOTLIB_ERR}")
        if not WORDCLOUD_OK:
            st.caption(f"wordcloud no disponible: {WORDCLOUD_ERR}")
        st.stop()
