# app.py — Panel de tabulados, cruces, mapa GPS, texto abierto y exportación
...
# ====== Estilos (tabs con espacio y salto de línea si no caben) ======
st.markdown("""
<style>
...
# ====== Faltantes (global) — no se tabulan ======
MISSING_LABELS = {
...
def vc_percent(df, col, by=None):
    """Conteos y % excluyendo etiquetas de faltantes definidas en MISSING_LABELS."""
    def cat(s):
...
def crosstab_pct(df, r, c, by=None):
    """Cruces con conteos y % por fila; excluye MISSING_LABELS y quita n_fila en %."""
    def cat(s):
        return s.astype("object").where(s.notna(), "(Sin dato)").astype(str)
...
if sector != "<ninguna>":
    ...
    sel = st.sidebar.multiselect("Sector", options=vals, default=vals, key="filtro_sector")
...
with tabI:
    ...
    if sexoj!="<ninguna>":
        ...
        ind_tables["% hogares con jefatura femenina"] = (s.str.contains("mujer") | s.str.contains("femen")).mean()*100 if len(s)>0 else np.nan
    if p010!="<ninguna>":
    ...
    if p015!="<ninguna>":
        ...
        ind_tables["% hogares sin acceso a agua potable"] = (~(s.str.contains("agua") | s.str.contains("acued"))).mean()*100 if len(s)>0 else np.nan
    if p018!="<ninguna>":
        s = base[p018].astype(str).str.lower()
        ind_tables["% hogares con saneamiento inadecuado"] = (s.str.contains("letrin") | s.str.contains("ninguno") | s.str.contains("compart")).mean()*100 if len(s)>0 else np.nan
    if p027!="<ninguna>":
        s = base[p027].astype(str).str.lower()
        ind_tables["% negocios sin permisos"] = (s.str.contains("no") | s.str.contains("ninguno")).mean()*100 if len(s)>0 else np.nan
    if p022!="<ninguna>":
...
with tabTXT:
    ...
    n_top = st.slider("Top términos a mostrar", 10, 50, 20, key="txt_topn_abiertas")
    ...
    col_wc = st.selectbox("Selecciona columna para la nube", options=text_cols, index=0,
                          key="txt_wc_select_abiertas")
    ...
    if len(txt_wc) < 3:
        ...
        try:
            ...
            if img is None:
                ...
                st.image(img, use_column_width=True)  # compatible
        except Exception as e:
...
otro: otro|varios|misc"""
    dict_text = st.text_area("Diccionario (formato: categoria: palabra1|palabra2|...)", value=sample_dict,
                             height=150, key="dict_text_abiertas")
    ...
    col_to_code = st.selectbox("Columna a codificar", options=text_cols, index=0, key="sel_code_col_abiertas")
    ...
    if st.button("Aplicar codificación", use_container_width=True, key="btn_code_abiertas"):
...
- luego usa *Rerun/Reboot* en Streamlit.
...
"""
    try:
        ...
        if manual_path is not None:
            ...
            md = DEFAULT_MD; source = "Mostrando manual de respaldo (no se encontró MANUAL.md)."
...
with tabEXPORT:
    ...
    for col, key in [(p025,"F_p025"),(p026,"F_p026"),(p027,"F_p027"),(p028,"F_p028"),(p029,"F_p029"),
                     (p030,"F_p030"),(p031,"F_p031"),(p032,"F_p032")]:
        if col!="<ninguna>": sheets[key] = vc_percent(subF, col, by=sector if sector!='<ninguna>' else None)
    for r,c,key in [(p025,p027,"F_p025_x_p027"),(p027,p028,"F_p027_x_p028"),(p030,p029,"F_p030_x_p029"),
                    (p026,p027,"F_p026_x_p027"),(p031,p027,"F_p031_x_p027")]:
